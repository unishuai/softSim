...
class Ur10FreeHandSim(object):
    ...
    def moveArm(self, action, myMaxVelocity=-1):
        ...
        if myMaxVelocity < 0:
            for i, joint_id in enumerate(self.arm_controllable_joints):
                self.p.setJointMotorControl2(self.robotId, joint_id, self.p.POSITION_CONTROL,
                                             joint_poses[i],
    ...
    #     '''
    #     if orn is None:
    #         # orn = self.p.getQuaternionFromEuler([0,0,math.pi / 2])   # 机械手方向
    #         orn = self.p.getQuaternionFromEuler([0, 0, 0])  # 机械手方向
    #
    #
